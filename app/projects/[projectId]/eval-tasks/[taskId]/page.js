          ...
          <>
            ...
            <Box sx={detailStyles.paperContainer} id="paper-top">
              ...
              <Box sx={detailStyles.paperHeader}>
                <Typography sx={detailStyles.paperTitle}>模型能力评估报告</Typography>
                <Typography sx={detailStyles.paperSubTitle}>Model Capability Evaluation Report</Typography>
                <Box
                  sx={{
                    mt: 2,
                    display: 'flex',
                ...
                >
                  <Typography variant="caption">Task ID: {taskId}</Typography>
              ...
              <Box sx={{ p: 3, bgcolor: '#fff' }}>
                  ...
                  <Grid container spacing={3}>
                    {results?.map((result, index) => (
                      <Grid item xs={12} md={6} key={result.id}>
                        <QuestionCard result={result} index={(page - 1) * pageSize + index} />
                      </Grid>
                    ))}
                  </Grid>
                )}
                ...
                {!loading && results?.length === 0 && (
                  <Box sx={{ p: 8, textAlign: 'center', color: 'text.disabled' }}>
                    <Typography>暂无符合条件的评估结果</Typography>
                  </Box>
                )}
              </Box>
              ...
              <Box sx={{ p: 4, textAlign: 'center', color: 'text.disabled', borderTop: '2px solid #000' }}>
                <Typography variant="caption">Easy Dataset Evaluation System · Generated by AI</Typography>
              </Box>
            </Box>
          </>
        )}
