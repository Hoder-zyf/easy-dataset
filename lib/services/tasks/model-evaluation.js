...
export async function processModelEvaluationTask(task) {
  ...
  try {
    ...
    const { evalDatasetIds, judgeModelId, judgeProviderId } = taskDetail;
...
async function saveEvalResult(projectId, taskId, evalDatasetId, result) {
  const { modelAnswer, score, isCorrect, judgeResponse } = result;

  await prisma.evalResults.upsert({
    where: { taskId_evalDatasetId: { taskId, evalDatasetId } },
    update: { modelAnswer, score, isCorrect, judgeResponse },
    create: { projectId, taskId, evalDatasetId, modelAnswer, score, isCorrect, judgeResponse }
  });
}
