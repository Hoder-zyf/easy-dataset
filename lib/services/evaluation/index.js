...
function evaluateTrueFalse(modelAnswer, correctAnswer) {
  ...
  const truePatterns = ['true', '正确', '对', 't', 'yes', '是'];
  const falsePatterns = ['false', '错误', '错', 'f', 'no', '否'];

  const modelIsTrue = truePatterns.some(p => normalizedModel.includes(p));
  const modelIsFalse = falsePatterns.some(p => normalizedModel.includes(p));
  const correctIsTrue = truePatterns.some(p => normalizedCorrect.includes(p));
  const correctIsFalse = falsePatterns.some(p => normalizedCorrect.includes(p));

  const isCorrect = (modelIsTrue && correctIsTrue) || (modelIsFalse && correctIsFalse);
