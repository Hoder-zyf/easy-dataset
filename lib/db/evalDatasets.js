 ...
 */
export function buildEvalQuestionWhere(projectId, { questionType, keyword, chunkId, tags } = {}) {
...
export async function getEvalQuestionsWithPagination(projectId, options = {}) {
  try {
    const { page = 1, pageSize = 20, questionType, keyword, chunkId, tags } = options;
    const skip = (page - 1) * pageSize;

    // 构建查询条件
    const where = buildEvalQuestionWhere(projectId, { questionType, keyword, chunkId, tags });
